...
val ins=Instructions
	...
	if (formal){
		...
		when(io.state === ins.andi){    // AND
			verification.assert(io.out === (io.in1 & io.in2))
		}


		when(io.state === ins.slt){    //SMALLER THAN
			verification.assert(io.out === ((io.in1.asSInt < io.in2.asSInt).asUInt))
		}

		when(io.state === ins.slti){    // smaller than
			verification.assert(io.out === (io.in1.asSInt < io.in2.asSInt).asUInt)
		}

		when(io.state === ins.sltu){    // smaller than unsigned
			verification.assert(io.out === (Mux(io.in1 < io.in2, 1.U, 0.U)))
		}

		when(io.state === ins.sltiu){    // smaller than unsigned
			verification.assert(io.out ===(Mux(io.in1 < io.in2, 1.U, 0.U)))
		}



		when(io.state === ins.beq){	// equal to
			verification.assert(io.out === (Mux(io.in1 === io.in2, 1.U, 0.U)))
		}

		when(io.state === ins.bne){	// not equal to
			verification.assert(io.out === (Mux(io.in1 === io.in2, 0.U, 1.U)))
		}

		when(io.state ===ins.blt){	// smaller than
			verification.assert(io.out === (io.in1.asSInt < io.in2.asSInt).asUInt)
		}

		when(io.state === ins.bge){	// greater or equal to
			verification.assert(io.out === ((io.in1.asSInt < io.in2.asSInt).asUInt  ^ 1.U))
		}

		when(io.state === ins.bltu){	// smaller than unsigned
			verification.assert(io.out === (Mux(io.in1 < io.in2, 1.U, 0.U)))
		}

		when(io.state === ins.bgeu){   // greater or equal to unsigned
			verification.assert(io.out === (Mux(io.in1 < io.in2, 0.U, 1.U)))
		}
